...
def create_room_background(room_name):
    ...
    if room_name == "School Hall":
        ...
        # Door at top (leads back to hall) - ADD THIS
        pygame.draw.rect(surf, (100, 80, 60), (SCREEN_WIDTH//2-40, 100, 80, 40))
        pygame.draw.rect(surf, (80, 60, 40), (SCREEN_WIDTH//2-35, 105, 70, 30))
        pygame.draw.circle(surf, (200, 180, 100), (SCREEN_WIDTH//2, 120), 4)
...
ROOMS = {
    ...
    'spawn': (320, 300),
    'collision_rects': [
        pygame.Rect(0, 0, SCREEN_WIDTH, 1),  # Top edge
        pygame.Rect(0, 0, 1, SCREEN_HEIGHT),  # Left edge
        pygame.Rect(SCREEN_WIDTH-1, 0, 1, SCREEN_HEIGHT),  # Right edge
        pygame.Rect(0, SCREEN_HEIGHT-1, SCREEN_WIDTH, 1),  # Bottom edge
    ],
        'doors': {
            'buffet': {'rect': pygame.Rect(40, 180, 60, 80), 'target': 'buffet', 'spawn': (100, 240), 'reverse_door': 'hall'},
            'locker': {'rect': pygame.Rect(SCREEN_WIDTH-100, 180, 60, 80), 'target': 'locker', 'spawn': (540, 240), 'reverse_door': 'hall'},
            'bathroom': {'rect': pygame.Rect(160, SCREEN_HEIGHT-40, 60, 40), 'target': 'bathroom', 'spawn': (240, 200), 'reverse_door': 'hall'},
            'classroom': {'rect': pygame.Rect(360, SCREEN_HEIGHT-40, 60, 40), 'target': 'classroom', 'spawn': (320, 200), 'reverse_door': 'hall'}
        },
        'npcs': [
    ...
    'collision_rects': [
        ...
        'doors': {
            'hall': {'rect': pygame.Rect(SCREEN_WIDTH//2-40, 100, 80, 40), 'target': 'hall', 'spawn': (400, 300), 'reverse_door': 'classroom'}
        },
        'npcs': [
    ...
    'collision_rects': [
        ...
        'doors': {
            'hall': {'rect': pygame.Rect(40, 180, 60, 80), 'target': 'hall', 'spawn': (200, 300), 'reverse_door': 'locker'},
            'outside': {'rect': pygame.Rect(SCREEN_WIDTH-100, 180, 60, 80), 'target': 'outside', 'spawn': (540, 240), 'reverse_door': 'locker'}
        },
        'npcs': [
    ...
    'collision_rects': [
        ...
        'doors': {
            'hall': {'rect': pygame.Rect(SCREEN_WIDTH//2-40, 100, 80, 40), 'target': 'hall', 'spawn': (400, 300), 'reverse_door': 'bathroom'}
        },
        'npcs': [
    ...
    'collision_rects': [
        ...
        'doors': {
            'hall': {'rect': pygame.Rect(SCREEN_WIDTH-100, 180, 60, 80), 'target': 'hall', 'spawn': (200, 300), 'reverse_door': 'buffet'}
        },
        'npcs': [
    ...
    'collision_rects': [
        ...
        'doors': {
            'locker': {'rect': pygame.Rect(40, 180, 60, 80), 'target': 'locker', 'spawn': (200, 300), 'reverse_door': 'outside'}
        },
        'npcs': [
...
class Game:
    def __init__(self):
        ...
        # Room backgrounds will be created on demand
        ...
        self.last_door_used = None  # Track which door player came from
        self.nearby_door = None  # Track which door player is near
    ...
    def check_collision(self, x, y):
        ...
        # Check room boundaries ONLY (1-pixel borders)
    ...
    def check_spawn_collision(self, x, y):
        """Check collision for spawn positions (ignores NPCs)"""
        test_rect = pygame.Rect(x - PLAYER_SIZE//2, y - PLAYER_SIZE//2, PLAYER_SIZE, PLAYER_SIZE)
        room = ROOMS[self.current_room]
        
        # Check room boundaries only (not NPCs)
        for rect in room['collision_rects']:
            if test_rect.colliderect(rect):
                return True
    ...
    def check_door_transition(self):
        ...
        for door_name, door_data in room['doors'].items():
            if player_rect.colliderect(door_data['rect']):
                target_room = door_data['target']
                reverse_door_name = door_data.get('reverse_door')
                
                # Calculate spawn next to reverse door if available
                if reverse_door_name:
                    spawn = self.calculate_spawn_position(target_room, reverse_door_name)
                else:
                ...
                self.is_moving = False
                self.animation_frame = 0
                self.last_door_used = door_name
                self.play_sound('door')
                
                if self.current_room in self.room_backgrounds:
                    del self.room_backgrounds[self.current_room]
                ...
                # Interaction
                    ...
                    if event.key == pygame.K_e or event.key == pygame.K_RETURN:
                        ...
                        if npc:
                            ...
                            self.play_sound('select')
                        # Try door if no NPC
                        elif self.nearby_door:
                            self.teleport_through_door()
    ...
    def update(self):
        ...
        # Update nearby door detection
        self.update_nearby_door()
    
    def calculate_spawn_position(self, target_room, reverse_door_name):
        """Calculate spawn position next to the door player came from.
        Adds offset so player doesn't immediately re-trigger the door."""
        target_room_data = ROOMS[target_room]
        reverse_door = target_room_data['doors'].get(reverse_door_name)
        
        if not reverse_door:
            # Fallback to room spawn if reverse door not found
            return target_room_data['spawn']
        
        door_rect = reverse_door['rect']
        door_center_x = door_rect.centerx
        door_center_y = door_rect.centery
        
        # Determine door orientation and offset player away from it
        door_width = door_rect.width
        door_height = door_rect.height
        
        # Offset distance to keep player away from door
        offset = 80
        
        # Generate candidate spawn positions
        candidates = []
        
        # Vertical door (taller than wide)
        if door_height > door_width:
            if door_rect.centerx < SCREEN_WIDTH // 2:
                # Door on left side, spawn to the right
                candidates.append((door_rect.right + offset, door_center_y))
                candidates.append((door_rect.right + offset + 60, door_center_y))
            else:
                # Door on right side, spawn to the left
                candidates.append((door_rect.left - offset, door_center_y))
                candidates.append((door_rect.left - offset - 60, door_center_y))
        # Horizontal door (wider than tall)
        else:
            if door_rect.centery < SCREEN_HEIGHT // 2:
                # Door on top, spawn below
                candidates.append((door_center_x, door_rect.bottom + offset))
                candidates.append((door_center_x, door_rect.bottom + offset + 60))
            else:
                # Door on bottom, spawn above
                candidates.append((door_center_x, door_rect.top - offset))
                candidates.append((door_center_x, door_rect.top - offset - 60))
        
        # Try each candidate position, use first valid one (ignoring NPCs)
        for spawn_x, spawn_y in candidates:
            # Clamp to valid room bounds
            spawn_x = max(PLAYER_SIZE, min(SCREEN_WIDTH - PLAYER_SIZE, spawn_x))
            spawn_y = max(PLAYER_SIZE, min(SCREEN_HEIGHT - PLAYER_SIZE, spawn_y))
            
            # Check if this position is collision-free (only check boundaries, not NPCs)
            if not self.check_spawn_collision(spawn_x, spawn_y):
                return (spawn_x, spawn_y)
        
        # Fallback to room spawn if no safe position found
        return target_room_data['spawn']
    
    def update_nearby_door(self):
        """Check which door player is near (if any)"""
        room = ROOMS[self.current_room]
        
        self.nearby_door = None
        
        for door_name, door_data in room['doors'].items():
            # Check if player is near door (within 100 pixels)
            door_rect = door_data['rect']
            distance = math.sqrt((self.player_x - door_rect.centerx)**2 + (self.player_y - door_rect.centery)**2)
            
            if distance < 100:
                self.nearby_door = (door_name, door_data)
                return
    
    def teleport_through_door(self):
        """Teleport player through the nearby door"""
        if not self.nearby_door:
            return False
        
        door_name, door_data = self.nearby_door
        target_room = door_data['target']
        reverse_door_name = door_data.get('reverse_door')
    ...
    def draw_room(self):
        # Create background on demand if not cached
        if self.current_room not in self.room_backgrounds:
            self.room_backgrounds[self.current_room] = create_room_background(ROOMS[self.current_room]['name'])
        
        # Draw background
        self.screen.blit(self.room_backgrounds[self.current_room], (0, 0))
        
        # Draw door interaction prompts
        room = ROOMS[self.current_room]
        player_rect = self.get_player_rect()
        ...
        for door_name, door_data in room['doors'].items():
            ...
            if distance < 100:
                # Draw door prompt
                prompt = self.font_tiny.render("[E] To walk through doors", True, WHITE)
                prompt_bg = pygame.Rect(door_rect.centerx - 80, door_rect.centery - 40, 160, 20)
                pygame.draw.rect(self.screen, (0, 0, 0, 150), prompt_bg)
                self.screen.blit(prompt, (door_rect.centerx - prompt.get_width()//2, door_rect.centery - 38))
    ...
    def draw(self):
        if self.in_menu:
            ...
            # Clear the entire screen with black first
